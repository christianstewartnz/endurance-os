// Context — the athlete's editable AI intelligence system.
// Replaces Plan DNA / Coach Style / Athlete Brain.
...
const CONTEXT_MODULES = [
  {
    ...
    fields: [
      ['Name', 'Mira Lindqvist'],
      ['Age / Sex', '34 · Female'],
      ['Discipline', 'Triathlon · Long course (70.3, IM)'],
      ['Experience', '6 years racing · 4 years self-coached'],
      ['Strengths', 'Bike threshold, mental durability'],
      ['Weaknesses', 'Open-water sighting · running off the bike past 18 km'],
    ],
    edited: '2 weeks ago',
  },
  {
    ...
    fields: [
      ['Tone', 'Direct'],
      ['Length', 'Short. State the recommendation, then briefly why.'],
      ['Praise', 'Minimal. Numbers speak.'],
      ['Push vs protect', 'Bias toward protecting form during build blocks.'],
      ['When to challenge me', 'Always, when readiness data contradicts my plan.'],
    ],
    edited: 'Yesterday',
  },
  {
    ...
    fields: [
      ...
      ['Quality days', 'Tue + Thu (48h between hard)'],
      ['Long ride', 'Saturday — fixed'],
      ['Long run', 'Sunday brick'],
      ['Ramp rate', '4–6 TSS/wk'],
      ['Volume target', '12–16 h/wk in build'],
    ],
    edited: '4 days ago',
  },
  {
    ...
    fields: [
      ...
      ['VO2 stacking', 'Athlete struggles with stacked VO2 after long endurance weekends.', { pending: true }],
    ],
    edited: '6 days ago',
    pending: 1,
  },
  {
    ...
    fields: [
      ['HRV trigger', 'IF HRV < −7% for 2 days → propose Z2 swap'],
      ['Sleep trigger', 'IF sleep < 6h → propose intensity downshift'],
      ['Travel', 'IF >2h flight in window → cap intensity 24h post'],
      ['Race week', 'No Z4+ work in T-7. Openers Thu.'],
      ['Sick / injured', 'Auto-pause for 48h. Resume Z1 only.'],
      ['Apply mode', 'Auto-propose · always ask before applying.'],
    ],
    edited: '2 days ago',
  },
  {
    ...
    fields: [
      ['A-race', 'Ironman 70.3 Lahti · Aug 16'],
      ['Overall goal', 'Sub 4:55 · top 5 AG'],
      ['Bike target', '2:38 @ NP 215 W (0.79 IF)'],
      ['Run target', '1:35 (4:30/km off the bike)'],
  ...
  },
  {
    ...
    fields: [
      ['Race fuel', '90 g carb/h (gel + drink mix)'],
      ['Long ride fuel', '70 g/h · bars allowed in first 90 min only'],
      ['Pre-race', 'Oats + banana + coffee, T-3h'],
      ['Hydration', '750 ml/h · 600 mg sodium/h hot'],
      ['Caffeine', '200 mg, split T-0 and T+90 on race day'],
    ],
    edited: '3 days ago',
  },
  {
    id: 'health',
    title: 'Health & Injury Notes',
    icon: 'heart',
    tag: '@health',
    summary: 'Left Achilles flare history · monitor on consecutive run days',
    fields: [
      ['Active', 'None'],
      ['History', 'Left Achilles tendinopathy · 2023 winter. 6 wk loaded eccentrics resolved.'],
  ...
  },
  {
    ...
    fields: [
      ['Sleep target', '7h 30m'],
      ['Rest day', 'Sunday preferred · Friday if travel'],
      ['Modalities', 'Sauna 2×/wk · light Z1 spin OK on rest'],
      ['HRV measurement', 'Garmin · wrist · upon wake'],
      ['Off-week cadence', 'Every 4th week, 60% load'],
    ],
    edited: '1 week ago',
  },
];
...
function ContextView() {
  const [activeId, setActiveId] = useStateCtx('plan-dna');
  const [editingId, setEditingId] = useStateCtx(null);
  const active = CONTEXT_MODULES.find(m => m.id === activeId);
        ...
        <div style={{
          ...
          {CONTEXT_MODULES.map(m => (
            <div
              ...
              onMouseEnter={e => { if (activeId !== m.id) e.currentTarget.style.background = 'var(--bg-1)'; }}
            ...
            >
              ...
              <span style={{
                flex: 1, fontSize: 13,
                color: activeId === m.id ? 'var(--fg-1)' : 'var(--fg-2)',
                fontWeight: activeId === m.id ? 500 : 400,
              }}>{m.title}</span>
              {m.pending && (
                <span style={{
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '8px 0' }}>
        {module.fields.map(([k, v, opts], i) => (
          <div key={k} style={{
            display: 'grid',
            gridTemplateColumns: '180px 1fr',
            gap: 16,
            padding: '14px 24px',
            borderBottom: i < module.fields.length - 1 ? '1px solid var(--border-subtle)' : 'none',
            alignItems: 'baseline',
            background: opts && opts.pending ? 'rgba(139,124,246,0.04)' : 'transparent',
          }}>
            <div style={{
              fontSize: 11,
              ...
              {k}
              {opts && opts.pending && (
                <span style={{ fontSize: 9, color: 'var(--ai)', fontFamily: 'var(--font-mono)', textTransform: 'none', letterSpacing: 0 }}>pending</span>
              )}
            </div>
            {editing ? (
              <textarea
              ...
              <div style={{ fontSize: 13, color: opts && opts.pending ? 'var(--ai)' : 'var(--fg-1)', lineHeight: 1.55 }}>
                {v}
                {opts && opts.pending && (
  ...
  const items = [
    {
      ...
      tag: '@patterns',
      text: 'Athlete struggles with stacked VO2 sessions after long endurance weekends.',
      evidence: 'Observed Wks 4, 7, 10 · 3 missed VO2 sets · avg 36h after long ride',
      source: 'auto · last 3 build blocks',
    },
    {
      target: 'Fueling Strategy',
      tag: '@fueling',
      text: 'Switch from bars to gels after 90 min on long rides — gut tolerance drops.',
      evidence: '3 of 4 long rides reported GI discomfort with bars past 90 min',
      source: 'Sat May 31 · debrief conversation',
    },
    {
      target: 'Recovery Preferences',
      tag: '@recovery',
      text: 'Add sauna restriction after VO2 days — reported elevated next-morning RHR (+3 bpm).',
      evidence: 'Last 2 cycles, sauna post-VO2 → RHR +3 bpm next morning',
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--fg-3)' }}>
            Coach proposed updates to your context. Nothing applies until you accept.
          </div>
        </div>
