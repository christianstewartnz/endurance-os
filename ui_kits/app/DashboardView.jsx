// Dashboard view: today's session, readiness, week strip, recent reviews.
const { useState: useStateD } = React;

function TodaysSessionCard({ onOpenCoach }) {
  const [adaptation, setAdaptation] = useStateD('pending'); // pending | accepted | rejected
  return (
    <div style={{
      background: 'var(--bg-2)',
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            {adaptation === 'accepted' && (
              <Pill color="ai"><Icon name="sparkles" size={10} style={{ marginRight: 2 }} />Adapted to Z2</Pill>
            )}
          </div>
          <div style={{ fontSize: 28, fontWeight: 600, letterSpacing: '-0.02em', lineHeight: 1.15, color: 'var(--fg-1)', textWrap: 'pretty' }}>
            {adaptation === 'accepted' ? 'Endurance · 90 min Z2' : 'Threshold intervals · 4 × 8 min'}
          </div>
          <div style={{ fontSize: 14, color: 'var(--fg-2)', marginTop: 8, lineHeight: 1.55, maxWidth: 520 }}>
            {adaptation === 'accepted' ? (
              <>Steady at <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--fg-1)' }}>180–210 W</span> · cadence
              <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--fg-1)' }}> 88–92</span>.
              Threshold moves to Thursday.</>
            ) : (
              <>Hold <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--fg-1)' }}>285 W</span> · cadence
              <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--fg-1)' }}> 92</span>. 4 min easy between.
              Wind 6 m/s SW — bias the steady direction for the long efforts.</>
            )}
          </div>
        ...
        <div style={{
          ...
          {adaptation === 'accepted' ? <Z2Profile /> : <WorkoutProfile />}
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--fg-3)', fontFamily: 'var(--font-mono)' }}>
            ...
            <span>{adaptation === 'accepted' ? '1:30' : '1:42'}</span>
          </div>
        </div>
      </div>

      {/* Adaptation suggestion strip */}
      {adaptation === 'pending' && (
      ...
      {adaptation === 'rejected' && (
        <div style={{
          ...
          Suggestion declined. Keeping threshold as planned. <button onClick={() => setAdaptation('pending')} style={{ marginLeft: 'auto', background: 'transparent', border: 'none', color: 'var(--fg-3)', fontSize: 11, cursor: 'pointer', textDecoration: 'underline' }}>Show again</button>
        </div>
      )}
    </div>
  );
...
function AdaptationSuggestion({ onAccept, onReject, onDiscuss }) {
  return (
    <div style={{
      borderTop: '1px solid var(--ai-edge)',
      background: 'linear-gradient(180deg, rgba(139,124,246,0.05), rgba(139,124,246,0.02))',
      padding: '16px 24px',
      display: 'flex',
      gap: 14,
    ...
    <div style={{ height: 120, display: 'flex', alignItems: 'flex-end', gap: 1 }}>
      {[
        ...
        <div key={i} style={{
          flex: b.w,
          height: `${b.p * 100}%`,
          background: 'linear-gradient(180deg, #3FB37F55, #3FB37Faa)',
          borderTop: '2px solid #3FB37F',
...
function WorkoutProfile() {
  // Warmup (low), 4 x (high 8min, low 4min), cooldown
  const blocks = [
    { w: 12, p: 0.38, z: 'z2' },
    { w: 16, p: 0.95, z: 'z4' },
    { w: 8,  p: 0.45, z: 'z2' },
  ...
  const colors = { z2: '#3FB37F', z3: '#E8C547', z4: '#E89B3C', z5: '#E5484D' };
    ...
    <div style={{ height: 120, display: 'flex', alignItems: 'flex-end', gap: 1 }}>
      {blocks.map((b, i) => (
        <div key={i} style={{
          flex: b.w,
          height: `${b.p * 100}%`,
          background: `linear-gradient(180deg, ${colors[b.z]}55, ${colors[b.z]}aa)`,
          borderTop: `2px solid ${colors[b.z]}`,
          borderRadius: '2px 2px 0 0',
        }} />
      ))}
    </div>
...
function ReadinessRow() {
  const cards = [
    { k: 'HRV', v: '64', u: 'ms', delta: '−8% · 14d', tone: 'warning', spark: [72,70,68,71,69,67,68,66,70,68,65,66,64,64], baseline: 70 },
    { k: 'Resting HR', v: '48', u: 'bpm', delta: '+2 · 7d', tone: 'warning', spark: [46,45,47,45,46,47,46,48,47,49,48,48,49,48], baseline: 46 },
    { k: 'Sleep', v: '6:12', u: 'h', delta: '−1:18 · target', tone: 'danger', spark: [7.4,7.8,7.2,8.1,7.5,7.2,6.9,7.1,7.0,6.5,6.8,6.2,5.9,6.2], baseline: 7.5 },
    { k: 'Form (TSB)', v: '−12', u: '', delta: 'Building', tone: 'neutral', spark: [4,2,-1,-3,-2,-5,-7,-6,-8,-10,-9,-11,-12,-12], baseline: 0 },
  ];
            ...
            <div key={day.d} style={{
              ...
              {day.zone ? (
                <div style={{
                  ...
                  border: '1px solid var(--border-default)',
                  borderRadius: 6,
                  borderLeft: `2px solid ${zColors[day.zone]}`,
...
function MemoryInbox({ onNavigate }) {
  const items = [
    {
    ...
    },
    {
      ...
      text: 'Gut tolerance drops on bars after 90 min — switch to gels for second half of long rides.',
      source: 'Sat ride · May 31',
    },
  ];
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '18px 20px 12px' }}>
        ...
        <Button kind="ghost" size="sm" iconRight="arrow-right" onClick={() => onNavigate && onNavigate('context')}>Open Context</Button>
      </div>
      {items.map((it, i) => (
        <div key={i} style={{
          padding: '14px 20px',
...
function DashboardView({ onOpenCoach, onNavigate }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <TodaysSessionCard onOpenCoach={onOpenCoach} />
      <ReadinessRow />
      <WeekStrip />
      <MemoryInbox onNavigate={onNavigate} />
    </div>
  );
}

Object.assign(window, { DashboardView, TodaysSessionCard, ReadinessRow, WeekStrip, MemoryInbox });
